...
1. 开箱即用的GraphQL接口
2. 轻量级无依赖,不依赖于如Spring,Mybatis等基础框架
3. 无需异步编程知识,同步代码自动编译为异步
4. 运行时无反射,编译增强
5. 单体与分布式自由切换的渐进式架构
6. 多类型数据库支持：SQL,KV...
            ...
            |-- main
                ...
                |-- resources
                    |-- graphql
                        |-- order.gql         定义其他相关类型
```
如上所示,app项目引入order和other两个包,如同货轮(app)和集装箱(order,other),此种架构的原因将在后面的章节说明.
...
3. 定义GraphQL Schema,关于GraphQL的基础知识,可以参考[GraphQL官方教程](https://graphql.org/learn/)
...
![generateGraphQLSource](/img/quick-start/generateGraphQLSource.png "generateGraphQLSource")
...
dependencies {
    ...
    implementation 'org.mariadb:r2dbc-mariadb:1.1.4' //mariadb
